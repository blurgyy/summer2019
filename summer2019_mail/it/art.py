...
class article:
    ...
    def parse(self, ):
        ...
        self.title = re.findall(
            r'<div class=[\'\"]content-tit+le[\'\"]>[\s\S]*?h1>(.*?)<\/h1>',
            html)[0]
        self.header = re.findall(
            r'<div class=[\'\"]content-sign[\'\"]>([\s\S]*?<\/div>)', html)[0]
        self.body = re.findall(
            r'<div class=[\'\"]content-article[\'\"]>([\s\S]*?)<\/form>',
            html)[0]
        links = set(re.findall(r'\"(/_.*?)\"', self.body))
        for link in links:
            self.body = re.sub(link, misc.url_join(self.base_url, link),
                               self.body)
