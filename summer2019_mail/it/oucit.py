...
class oucit:
    ...
    def parse(self, ):
        ...
        if (self.checking_list['news']):
            news = re.findall(r'(<div id="wp_news_w8">[\s\S]*?</div>)',
                              html)[0]
            news = re.findall(
                r'<a href=[\'\"](.*?)[\'\"] target=[\'\"]_blank[\'\"] title=[\'\"].+[\'\"]>.*?<\/a>',
                news)
            self.news = [misc.url_join(self.base_url, x) for x in news]
            assert len(self.news) == len(list(set(
                self.news))), "at least 1 news article duplicated"
        if (self.checking_list['notice']):
            notice = re.findall(r'(<div id="wp_news_w12">[\s\S]*?</div>)',
                                html)[0]
            notice = re.findall(
                r'<a href=[\'\"](.*?)[\'\"] target=[\'\"]_blank[\'\"] title=[\'\"].+[\'\"]>.*?<\/a>',
                notice)
            self.notice = [misc.url_join(self.base_url, x) for x in notice]
            assert len(self.notice) == len(list(set(
                self.notice))), "at least 1 notice article duplicated"
        if (self.checking_list['activity']):
            activity = re.findall(r'(<div id="wp_news_w14">[\s\S]*?</div>)',
                                  html)[0]
            activity = re.findall(
                r'<a href=[\'\"](.*?)[\'\"] target=[\'\"]_blank[\'\"] title=[\'\"].+[\'\"]>.*?<\/a>',
                activity)
            self.activity = [
                misc.url_join(self.base_url, x) for x in activity
            ]
            assert len(self.activity) == len(list(set(
                self.activity))), "at least 1 activity article duplicated"
    ...
    def check(self, ):
        ...
        if (self.checking_list['news']):
            ...
            if (os.path.exists(fname)):
                ...
                ret += [
                    self.news[i] for i in range(self.default_refresh_count)
                ]
        ...
        if (self.checking_list['notice']):
            ...
            if (os.path.exists(fname)):
                ...
                ret += [
                    self.notice[i] for i in range(self.default_refresh_count)
                ]
            with open(fname, 'w') as f:
        ...
        if (self.checking_list['activity']):
            ...
            if (os.path.exists(fname)):
                ...
                ret += [
                    self.activity[i] for i in range(self.default_refresh_count)
                ]
            with open(fname, 'w') as f:
