...
class cmdy(object):
    ...
    def get_list(self, html_text):
        index_area = re.findall(r'index-area clearfix([\s\S]*?)</div>',
                                html_text)[0]
        results = re.findall(r'<li([\s\S]*?)</li>', index_area)
        info = [
            re.findall(
                r'link-hover[\'\"] href=[\'\"](.*?)[\'\"] title=[\'\"](.*?)[\'\"]',
    ...
    class itempage(object):
        def __init__(self, item):
            ...
            self.m3u8info = []
            videourl_list = re.findall(
                r'videourl clearfix[\'\"]([\s\S]*?)</div>',
                misc.r_get(self.url))
            self.links_info = []
            for vlist in videourl_list:
                self.links_info.extend(
                    re.findall(
                        r'title=[\'\"](.*?)[\'\"][\s\S]*?href=[\'\"](.*?)[\'\"]',
                        vlist))
            if (len(self.links_info) > 0):
                html_text = misc.r_get(self.links_info[0][1])
                self.req = self.info['self'].host + re.findall(
                    r'src="(/upload/playdata/\d+/\d+/\d+\.js)"', html_text)[0]
        ...
        def pull(self, ):
            ...
            m3u8_list = re.findall(r'mac_url=unescape\(\'(.*?)\'\)',
                                   js_text)[0].split('#')
            m3u8info = [(x.split('$')[0], x.split('$')[1]) for x in m3u8_list]
            self.m3u8info = [{
                **self.info, 'fname': misc.unescape(x[0]) + ".m3u8",
                'hls_url': x[1]
