...
class mjw(object):
    ...
    def get_list(self,):
        ...
        try:
            content_block = re.findall(
                r'list-content([\s\S]*?)widget widget_hot', html_text)[0]
        except IndexError:
            content_block = html_text
        results = re.findall(r'(<article class="u-movie">[\s\S]*?</article>)',
                             content_block)
    ...
    class itempage(object):
        ...
        def __init__(self, item):
            ...
            vlists_info = re.findall(
                r'<div id="video_list_li" class="video_list_li">([\s\S]*?)</div>',
                misc.r_get(self.url))
            self.links_info = []
            for vlist in vlists_info:
                if ("vlink" in vlist):
                    ...
                    links_info = [
                        ...
                        for x in re.findall(r'<a.*?href="(.*?)">(.*?)</a>',
                                            vlist)
                    ]
